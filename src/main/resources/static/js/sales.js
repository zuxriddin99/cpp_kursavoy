...
class="product-image-home img-thumbnail px-auto" alt="Image not loaded..."/></a>`
            + '</div>'
            + `<h5 class="text-center font-weight-bold mt-3">Название: ${product.product.name}</h5>`
            + `<h5 class="text-center font-weight-bold" style="color: black">Цена: $${product.product.price.toFixed(2).strike()}</h5>`
            + `<h5 class="text-center font-weight-bold" style="color: orangered">Новая цена: $${product.price.toFixed(2)}</h5>`
            + '</div>'
    }
